...
const NavItem: React.FC<{
  ...
  colorClass?: string;
  badge?: string;
}> = ({ active, onClick, icon, label, colorClass = 'blue', badge }) => {
  const activeClasses = {
  ...
  return (
    <button onClick={onClick} className={`w-full flex items-center px-4 py-3 rounded-2xl border transition-all duration-200 group ${active ? activeClasses : 'border-transparent text-gray-500 hover:bg-gray-50 hover:text-blue-600'}`}>
      ...
      <span className="font-black text-[14px] flex-1 text-right">{label}</span>
      {badge && <span className="bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full animate-pulse">{badge}</span>}
    </button>
  );
};
...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen bg-[#FDFDFF] font-['Cairo'] flex text-right overflow-hidden relative" dir="rtl">
      <MotivationalToast />
      {adminBroadcast && (
        <div className="fixed top-0 left-0 right-0 z-[1000] bg-red-600 text-white p-4 text-center font-black animate-slide-up shadow-xl">
      ...
      <aside className={`fixed inset-y-0 right-0 z-50 w-72 bg-white border-l border-gray-100 flex flex-col transition-transform lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        <div className="p-8 flex flex-col h-full overflow-y-auto no-scrollbar">
          ...
          <nav className="space-y-1.5 flex-1">
            ...
            <button onClick={() => { setShowLiveTutor(true); setIsSidebarOpen(false); }} className="w-full mt-6 flex items-center px-4 py-3.5 rounded-2xl bg-indigo-600 text-white shadow-lg group hover:scale-[1.02] transition-transform">
