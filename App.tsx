...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        // Here you would ideally fetch extra profile data from a database
        setUser({
          uid: firebaseUser.uid,
          name: firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'طالب',
          email: firebaseUser.email || '',
          stream: 'علوم تجريبية', // Default or from DB
          xp: 2450,
          streak: 7,
          avatarSeed: firebaseUser.uid,
          joinDate: firebaseUser.metadata.creationTime || new Date().toISOString(),
          rank: firebaseUser.email === 'nacero1234@gmail.com' ? 'المشرف العام' : 'طالب متميز'
        });
      } else {
        setUser(null);
      }
    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-[#FDFDFF]">
        <div className="text-center">
          <div className="w-20 h-20 bg-blue-600 rounded-3xl flex items-center justify-center shadow-2xl mb-4 mx-auto rotate-3">
