...
import { 
  LogIn, Globe, Mail, Lock, Sparkles, Eye, EyeOff, ChevronLeft, AlertTriangle
} from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onComplete }) => {
  ...
  const parseError = (err: any) => {
    ...
    if (err.code === 'auth/operation-not-allowed') {
      return { 
        message: "عذراً! يجب تفعيل (Email/Password) في لوحة تحكم Firebase (Authentication > Sign-in method) أولاً ليعمل التسجيل.",
        code: err.code 
      };
    }
    if (err.code === 'auth/email-already-in-use') return { message: "هذا البريد الإلكتروني مسجل مسبقاً." };
    if (err.code === 'auth/weak-password') return { message: "كلمة المرور يجب أن تكون 6 أحرف على الأقل." };
    if (err.code === 'auth/invalid-credential') return { message: "بيانات الدخول غير صحيحة." };
    return { message: "حدث خطأ غير متوقع: " + err.message };
  };
        ...
        {view !== 'stream' ? (
          <div className="w-full bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-8 border border-white animate-in zoom-in-95 duration-500">
            ...
            <form onSubmit={view === 'login' ? onLoginSubmit : (e) => { e.preventDefault(); setView('stream'); }} className="space-y-5">
              ...
              {error && (
                <div className="p-4 bg-red-50 text-red-600 rounded-2xl border border-red-100 flex gap-3 items-start animate-slide-up">
                  <AlertTriangle size={20} className="shrink-0 mt-1" />
                  <div>
                    <p className="text-xs font-black mb-1">تنبيه النظام:</p>
