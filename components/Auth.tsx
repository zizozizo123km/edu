...
const Auth: React.FC<AuthProps> = ({ onComplete }) => {
  ...
  const handleAdminQuickLogin = async () => {
    setIsLoading(true);
    try {
      const userCredential = await signInWithEmailAndPassword(auth, 'nacero1234@gmail.com', 'adminadmin');
      ...
      setError('خطأ في الدخول السريع: ' + err.message);
      setIsLoading(false);
    }
  };
  ...
  const onLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (validateLogin()) {
      ...
      try {
        ...
        setError('خطأ في تسجيل الدخول: ' + (err.code === 'auth/user-not-found' ? 'المستخدم غير موجود' : 'كلمة المرور خاطئة'));
        setIsLoading(false);
      }
    }
  };
