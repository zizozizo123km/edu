...
import { 
  LogIn, Globe, User, Mail, Lock, 
  ArrowRight, Sparkles, Eye, EyeOff, AlertTriangle, Loader2, GraduationCap
} from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onComplete }) => {
  ...
  const handleStreamSelect = async (stream: StreamType) => {
    ...
    try {
      ...
      setError(err.message.includes('email-already-in-use') ? "هذا البريد مسجل بالفعل." : "حدث خطأ أثناء إنشاء الحساب.");
      setView('signup');
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-full bg-[#FDFDFF] flex flex-col items-center justify-center p-6 relative font-['Cairo']" dir="rtl">
      <div className="absolute top-0 right-0 w-full h-1/2 bg-gradient-to-b from-blue-600 to-indigo-700 rounded-b-[4rem] z-0 shadow-2xl"></div>

      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-10 text-white">
          <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center shadow-2xl mb-4 mx-auto rotate-3 transition-transform hover:rotate-0">
             <Globe size={40} className="text-blue-600" />
          </div>
          <h1 className="text-4xl font-black">DzairEdu <span className="text-blue-200">Pro</span></h1>
          <p className="text-blue-100 font-bold mt-2">منصة التفوق في البكالوريا 🇩🇿</p>
        </div>
        ...
        {view !== 'stream' ? (
          <div className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-8 border border-white animate-in zoom-in-95 duration-500">
            <h2 className="text-2xl font-black text-gray-800 mb-8">
              {view === 'login' ? 'مرحباً بعودتك 👋' : 'أنشئ حسابك الآن 🚀'}
            </h2>
            ...
            <form onSubmit={view === 'login' ? handleLogin : (e) => { e.preventDefault(); setView('stream'); }} className="space-y-5">
              ...
              {error && (
                <div className="p-3 bg-red-50 text-red-600 rounded-xl text-xs font-black text-center flex items-center justify-center gap-2 border border-red-100">
              ...
              <button type="button" onClick={() => setView(view === 'login' ? 'signup' : 'login')} className="w-full text-center text-xs font-bold text-gray-400 hover:text-blue-600 transition-colors">
                {view === 'login' ? 'ليس لديك حساب؟ اشترك مجاناً' : 'لديك حساب بالفعل؟ سجل دخولك'}
              </button>
            </form>
          </div>
        ) : (
          <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl animate-in slide-in-from-bottom duration-500">
             <h2 className="text-2xl font-black text-gray-800 mb-6 text-center">اختر شعبتك الدراسية 🎓</h2>
             <div className="grid grid-cols-1 gap-3">
                {Object.keys(STREAM_SUBJECTS).map((stream) => (
                  <button key={stream} onClick={() => handleStreamSelect(stream as StreamType)}
                    className="w-full bg-gray-50 rounded-2xl p-5 flex items-center justify-between border-2 border-transparent hover:border-blue-400 hover:bg-blue-50 transition-all font-black text-gray-700"
                  >
                    <div className="flex items-center gap-3">
                      <GraduationCap className="text-blue-600" />
                      {stream}
                    </div>
                    <ArrowRight size={18} className="rotate-180 text-gray-300" />
                  </button>
                ))}
             </div>
          </div>
