...
import { 
  Heart, MessageSquare, Send, Edit2, Check, X, 
  MoreHorizontal, Sparkles, User, Trash2
} from 'lucide-react';
...
const PostsFeed: React.FC<PostsFeedProps> = ({ user, posts }) => {
  ...
  const isOwner = (post: Post) => (post as any).authorUid === auth.currentUser?.uid;

  return (
    <div className="space-y-8">
      {/* Create Post Area */}
      <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-gray-100 transition-all focus-within:border-blue-100">
        <textarea 
          placeholder={`أهلاً ${user.name.split(' ')[0]}! شارك شيئاً مفيداً مع زملائك...`}
          className="w-full bg-gray-50 rounded-2xl p-6 outline-none focus:bg-white border-2 border-transparent focus:border-blue-50 min-h-[120px] resize-none font-bold text-right transition-all shadow-inner"
          value={newContent}
          onChange={(e) => setNewContent(e.target.value)}
        />
        <div className="flex justify-end mt-4">
          <button onClick={handlePost} disabled={!newContent.trim()} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl hover:bg-blue-700 active:scale-95 disabled:opacity-50 flex items-center gap-2 transition-all">
            انشر في المجتمع <Send size={18} className="rotate-180" />
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {posts.map(post => (
          <div key={post.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden animate-slide-up group">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <div className="text-right">
                    <h4 className="font-black text-gray-900 flex items-center gap-2">
                      {post.author}
                      {isOwner(post) && <span className="text-[9px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full border border-blue-100 uppercase tracking-widest font-black">أنت</span>}
                    </h4>
            ...
            <div className="px-8 py-4">
              {editingPostId === post.id ? (
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 group-hover:bg-slate-100/50 transition-colors">
            ...
            <div className="px-8 py-4 bg-gray-50/50 flex items-center justify-between border-t border-gray-50">
              <div className="flex gap-6">
                <button onClick={() => handleLike(post)} className="flex items-center gap-2 text-rose-500 font-black hover:scale-110 transition-all">
                  <Heart size={20} fill={(post.likes || 0) > 0 ? "currentColor" : "none"} /> {post.likes || 0}
                </button>
                <button onClick={() => setExpandedComments(prev => prev.includes(post.id) ? prev.filter(id => id !== post.id) : [...prev, post.id])} className="flex items-center gap-2 text-indigo-600 font-black hover:text-indigo-700 transition-all">
              ...
              <div className="p-8 bg-gray-50 border-t border-gray-100 space-y-4 animate-in slide-in-from-top-4 duration-300">
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-2 font-bold outline-none focus:border-blue-400 transition-all shadow-sm" 
                    ...
                    return (
                      <div key={cid} className="flex gap-3 bg-white p-3 rounded-xl border border-gray-100 shadow-sm animate-in fade-in">
                        ...
                        <div className="flex-1 text-right">
                          <p className="text-[11px] font-black text-blue-600 leading-none mb-1">{c.author}</p>
