  ...
  return (
    <div className="space-y-10 animate-slide-up pb-24">
      ...
      <div className="relative bg-white rounded-[3.5rem] p-10 lg:p-14 border border-gray-100 shadow-xl overflow-hidden group">
        ...
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_20%_20%,rgba(59,130,246,0.03)_0%,transparent_50%)]"></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          <section className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-2xl font-black text-gray-800 flex items-center gap-3">
                <TrendingUp className="text-blue-600" size={28} /> حالة التقدم الأكاديمي
              </h2>
              <button className="text-blue-600 font-bold text-sm">التفاصيل</button>
            </div>
              ...
              {currentSubjects.map(s => (
                <div key={s.id} className="space-y-4">
                  ...
                  <div className="w-full h-3 bg-gray-50 rounded-full border border-gray-100 overflow-hidden">
                    <div 
                      className={`${s.color} h-full transition-all duration-1000 shadow-inner`} 
          ...
          <section className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
            <h2 className="text-2xl font-black text-gray-800 mb-10 flex items-center gap-3">
              <Award className="text-blue-600" size={28} /> إنجازاتي وشاراتي
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {achievements.map((ach, idx) => (
                <div key={idx} className="flex items-center gap-6 p-6 bg-gray-50/50 rounded-[2rem] border border-gray-100 hover:shadow-md transition-all group">
                  <div className={`w-16 h-16 ${ach.color} rounded-2xl flex items-center justify-center shadow-sm group-hover:rotate-12 transition-transform`}>
                    {ach.icon}
                  </div>
                  <div>
                    <h4 className="font-black text-gray-800 text-lg mb-1">{ach.title}</h4>
                    <p className="text-sm text-gray-400 font-bold">{ach.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Right Column: Stats & Target */}
        <div className="space-y-10">
          <div className="bg-gray-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              <div className="w-16 h-16 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center mb-8 border border-white/10 group-hover:rotate-12 transition-transform">
                <Target size={32} className="text-emerald-400" />
              </div>
              <h3 className="text-2xl font-black mb-4">هدفي القادم</h3>
              <p className="text-gray-400 font-bold mb-8 leading-relaxed italic">
                "أطمح للحصول على معدل 18.5 في البكالوريا والدخول لكلية الطب بجامعة الجزائر."
              </p>
              <div className="space-y-4">
                <div className="flex justify-between text-xs font-black">
                  <span className="text-gray-500">إنجاز الهدف</span>
                  <span className="text-emerald-400">45%</span>
                </div>
                <div className="w-full h-2.5 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 w-[45%] rounded-full shadow-lg shadow-emerald-500/20"></div>
                </div>
              </div>
            </div>
            <Target className="absolute bottom-[-30px] left-[-30px] w-48 h-48 opacity-5 -rotate-12 group-hover:rotate-0 transition-transform duration-700" />
          </div>

          <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
            <h3 className="font-black text-xl text-gray-800 mb-8 flex items-center gap-3">
              <Sparkles className="text-blue-600" size={24} /> إحصائيات عامة
            </h3>
            <div className="space-y-6">
              {[
                { label: 'ساعات الدراسة', value: '142 س', icon: <ChevronLeft size={16} /> },
                { label: 'تمارين محلولة', value: '382', icon: <ChevronLeft size={16} /> },
                { label: 'دروس مرئية', value: '24', icon: <ChevronLeft size={16} /> },
                { label: 'ترتيب الولاية', value: '12#', icon: <ChevronLeft size={16} /> }
              ].map((stat, i) => (
                <div key={i} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-all cursor-default group">
                   <span className="text-gray-500 font-bold flex items-center gap-2">
                     <span className="text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity">{stat.icon}</span>
                     {stat.label}
                   </span>
                   <span className="font-black text-gray-800">{stat.value}</span>
                </div>
              ))}
            </div>
          </div>
