    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden animate-slide-up rounded-3xl shadow-xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-[10px] font-bold text-blue-100">
              <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></span> طلاب متصلون الآن
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50">
        <div className="flex flex-col items-center mb-10 text-center">
           <div className="px-4 py-2 bg-white text-blue-600 rounded-full text-[10px] font-black border border-blue-50 flex items-center gap-2 mb-4 shadow-sm">
             <ShieldCheck size={14} /> فضاء آمن للتعاون الدراسي
           </div>
        </div>
          ...
          <div key={msg.id} className={`flex gap-4 ${msg.isMe ? 'flex-row-reverse' : ''} animate-in fade-in`}>
            ...
            <div className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'} max-w-[80%]`}>
              ...
              <div className={`px-5 py-3 rounded-2xl text-[14px] font-medium leading-relaxed shadow-sm border ${
                msg.isMe ? 'bg-blue-600 text-white rounded-tr-none shadow-blue-100' : 'bg-white text-gray-800 rounded-tl-none border-gray-100'
              }`}>
                {msg.content}
              </div>
      ...
      <div className="p-6 bg-white border-t border-gray-100">
        <div className="flex items-center gap-3 bg-gray-50 p-2 rounded-[1.5rem] border border-gray-200 focus-within:bg-white focus-within:border-blue-500 transition-all shadow-inner">
          <input 
            type="text" 
          ...
          />
          <button onClick={handleSendMessage} disabled={!inputValue.trim()} className="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg active:scale-95 disabled:opacity-50 transition-all">
